...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    ...(user?.role === 'admin' || user?.role === 'curator'
      ...
      : []),
    { href: '/entries/favorites', label: 'Favorites', exact: false },
    ...(user?.role === 'admin' ? [{ href: '/admin', label: 'Admin', exact: false }] : []),
  ]
  ...
  return (
    <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/80 sticky top-0 z-40 shadow-sm">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        {/* Logo */}
        <Link href="/entries" className="flex items-center gap-2 font-bold text-base tracking-tight">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div
                className="absolute right-0 mt-2 w-52 bg-white border border-gray-200 rounded-xl shadow-xl py-1.5 z-50 overflow-hidden"
                role="menu"
              >
                {/* User info */}
                ...
                <div className="py-1">
                  <Link
                    ...
                    className="flex items-center gap-2.5 w-full text-left px-3.5 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                    role="menuitem"
                  >
